...
function wordsList(str, subStr) {
  let strReplace = str.replace(/[.,!?;:"]/gi, '').toLowerCase().split(' ').filter(item => (item.indexOf(subStr) > -1))
  let colect = new Set()
...
function getLocalDate(date, isSeconds = false, isISO = false) {
  ...
  if (!isSeconds && !isISO) return date.toLocaleString().slice(0, -3)
  if (isSeconds && !isISO) return date.toLocaleString()
  if (!isSeconds && isISO) return result
  if (isSeconds && isISO) return `${result}:${seconds}`
}
...
function getWeekDay(date) {
  const myDate1 = new Date(date)
  switch (myDate1.getDay()) {
...
const getDateAgo = (date, days) => {
  ...
  return d.toLocaleString().slice(0, -10)
}
...
const Car = function (engine, model, name, year) {
  ...
  this.model = model;
  this.year = year;
  this.info = function () { return `${this.name} ${this.model}, ${this.engine}cc, year ${this.year}, ${this.used}` }
}
Object.defineProperties(Car.prototype, {
  used: {
    get() { return (yearNow > this.year) ? 'used' : 'new' },
    set(value) {
...
// const car = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
// const car2 = new Car(5000, 'FX50 AWD', 'Infinite', 2023);
